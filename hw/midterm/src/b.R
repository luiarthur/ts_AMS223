...
lines(1:N,theta.mean,lty=1,lwd=3,col='blue')
color.btwn(1:N,theta.ci[1,],theta.ci[2,],from=1,to=N,col.area=rgb(0,0,1,.2))

# One-step Ahead Forecast y_t | D_{t-1}
lines(1:N,theta.mean,lty=2,lwd=3,col='orange')
y.ci <- sapply(filt$param, function(p) 
               p$f + sqrt(p$Q) * qt(c(.025,.975),df=p$n-1))
color.btwn(1:N,y.ci[1,],y.ci[2,],from=1,to=N,col.area=rgb(1,.6,0,.3))

# Forecast (of y):
y_new <- forecast(filt,12)
fut_ind <- (N+1):(N+12)
points(fut_ind, y_new$f, col='darkgreen', pch=2,lwd=3,cex=1)
...
legend('bottomleft', 
       legend=c('Filtering', 'Smoothing', 'One-step Ahead', 
                '12-step Forecast (y)', 
                '12-step Forecast (theta)', 
                'Data'), bty='n', cex=2, 
       text.col=c('blue','red','orange','green', 'gold', 'grey30'),
       col=c('blue','red','orange','green', rgb(1,.9,0), 'grey30'),
       pch=c(NA,NA,NA,17,20,NA),
       text.font=2)
